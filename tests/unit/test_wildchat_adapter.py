"""Tests for the WildChat reader and manifest (skipped without pyarrow)."""
...
from longfeedback.config import WildChatDataConfig  # noqa: E402
from longfeedback.data.wildchat import (  # noqa: E402
    build_wildchat_manifest,
    iter_wildchat_records,
    prepare_wildchat,
)
...
def test_iter_records_reads_schema_flags_and_toxicity(tmp_path: Path) -> None:
    snapshot = _write_snapshot(tmp_path / "snap", with_cache_metadata=False)
    records = list(iter_wildchat_records(snapshot))
    ...
    # toxic=True also folds into "flagged" -- WildChat's toxic column is a
    # second, source-provided signal alongside OpenAI moderation.
    assert records[2].flagged
    assert records[4].redacted
...
def test_manifest_prefers_cache_metadata_and_falls_back_to_hashing(tmp_path: Path) -> None:
    with_cache = build_wildchat_manifest(_write_snapshot(tmp_path / "a", with_cache_metadata=True))
    assert with_cache.source_version == "fakecommit123"
    assert not with_cache.redistribute_raw_text

    without_cache = build_wildchat_manifest(
        _write_snapshot(tmp_path / "b", with_cache_metadata=False)
    )
...
def test_prepare_writes_artifacts_and_reconciled_stats(tmp_path: Path) -> None:
    ...
    config = WildChatDataConfig(input_dir=snapshot, output_dir=output, min_assistant_turns=3)
    result = prepare_wildchat(config)

    assert result.stats["conversations_scanned"] == 5
    ...
    assert kept_ids == {"wildchat-1m:keep-1", "wildchat-1m:keep-2"}

    # Determinism: a second run produces byte-identical artifacts.
    second = prepare_wildchat(config, output_dir=tmp_path / "processed2")
...
def test_prepare_rejects_empty_results(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="no conversations"):
        prepare_wildchat(config)


def test_missing_snapshot_directory_is_reported(tmp_path: Path) -> None:
    config = WildChatDataConfig(input_dir=tmp_path / "absent", output_dir=tmp_path / "out")
    with pytest.raises(FileNotFoundError, match="no WildChat parquet shards"):
        prepare_wildchat(config)
