"""WildChat-1M adapter: local snapshot to canonical trajectories.

WildChat is the project's *primary* conversational source (ODC-BY, ungated;
see docs/data_governance.md) -- LMSYS-Chat-1M is the secondary replication
source. Mirrors ``lmsys.py`` closely: same canonical conversion pipeline
(``longfeedback.data.conversations``), same synthetic-time scope decision
(the source has real per-turn timestamps, but E1 only needs turn order, so
this adapter does not thread them through -- a deliberate scope choice, not
an oversight). Raw and processed artifacts must stay under the gitignored
``data/`` tree even though ODC-BY permits redistribution in principle, to
keep one data-handling policy across all conversational sources. Requires
the ``research`` extra (pyarrow).
"""
...
_SOURCE_KEY = "wildchat-1m"
_SOURCE_URL = "https://huggingface.co/datasets/allenai/WildChat-1M"
_SOURCE_LICENSE = "ODC-BY 1.0 (Open Data Commons Attribution License)"


@dataclass(frozen=True)
class WildChatPrepareResult:
    stats: dict[str, Any]
...
def _shard_paths(input_dir: Path) -> list[Path]:
    ...
    if not shards:
        raise FileNotFoundError(
            f"no WildChat parquet shards under {input_dir / 'data'}; "
            "expected a HuggingFace snapshot of allenai/WildChat-1M"
        )
    return shards
...
def _row_toxic(row: dict[str, Any]) -> bool:
    return bool(row.get("toxic"))
...
def _record_from_row(row: dict[str, Any], *, shard: str, row_index: int) -> ConversationRecord:
    ...
    return ConversationRecord(
        ...
        flagged=_row_flagged(row.get("openai_moderation")) or _row_toxic(row),
...
def iter_wildchat_records(
    input_dir: Path, *, batch_size: int = 1_024
...
def _shard_checksums(input_dir: Path) -> tuple[dict[str, str], str | None]:
    ...
    Mirrors the LMSYS adapter: the HuggingFace download cache stores
    ``<etag-commit>\\n<sha256>\\n<time>`` metadata next to each shard when a
    snapshot was fetched via ``huggingface_hub``; hashing is the fallback for
    snapshots fetched by other means (e.g. a direct ``curl``).
    """
...
def build_wildchat_manifest(input_dir: Path) -> SourceManifest:
    ...
    return SourceManifest(
        ...
        derivative_license="not-redistributable (derived text stays local by project policy)",
...
def prepare_wildchat(
    config: WildChatDataConfig,
    *,
    input_dir: Path | None = None,
    output_dir: Path | None = None,
) -> WildChatPrepareResult:
    ...
    manifest = build_wildchat_manifest(resolved_input)
    ...
    for record in iter_wildchat_records(resolved_input):
        ...
        reason = conversation_exclusion_reason(
            ...
            exclude_flagged=config.exclude_flagged or config.exclude_toxic,
    ...
    stats: dict[str, Any] = {
        ...
        "caveats": {
            "synthetic_time": (
                "the source has real per-turn timestamps, but this adapter uses "
                "synthetic conversation-relative event times to match the LMSYS "
                "adapter's convention (E1 only needs turn order); real timestamps "
                "are dropped, a deliberate scope choice (see module docstring)"
            ),
            "user_leakage_unknown": (
                "the source has hashed IPs but no stable user identifier; the same "
                "person may appear in multiple splits"
            ),
            "split_strategy": "conversation-hash split (stable, but not chronological)",
        },
    }
